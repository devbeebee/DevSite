...
public static class NothingButMath
{
    public static Vector3 GetPositionAroundObject(this Vector3 tx, float radius,float y=0)
    {
        Vector3 offset = Random.insideUnitCircle * radius;
        Vector3 offsetv2 = new Vector3(offset.x,y,offset.y);
        Vector3 pos = tx + offsetv2;
        return pos;
    }
    public static Vector3 RandomPointInBox(this Vector3 center, Vector3 size)
    {

        return center + new Vector3(
           (Random.value - 0.5f) * size.x,0, (Random.value - 0.5f) * size.y
        );
    }
    //Have we passed a waypoint?
    //From http://www.habrador.com/tutorials/linear-algebra/2-passed-waypoint/
    public static bool HasPassedVector(this Vector3 carPos, Vector3 goingFromPos, Vector3 goingToPos)
    {
