...
public class CustomTagSystem
{
    public static Dictionary<int, List<GameObject>> m_taggedObjects = new Dictionary<int, List<GameObject>>();

    public static void Clear()=> m_taggedObjects.Clear();
    

    public static void AddObject(CustomTags tagObj)
    {
    ...
    public static void RemoveObject(CustomTags tagObj)
    {
        var tags = tagObj.GetTags();
        var go = tagObj.gameObject;
        foreach (var tag in tags)
